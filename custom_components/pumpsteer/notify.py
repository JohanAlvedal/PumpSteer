...
WATCHED = {
    "sensor.pumpsteer_is_braking":     ("🔴 Price braking active",   "Electricity is expensive — heating has been reduced."),
    "sensor.pumpsteer_is_preboosting": ("🔥 Pre-heating started",    "Electricity is cheap — pre-heating now before prices rise."),
}
...
def async_setup_notifications(hass: HomeAssistant, entry: ConfigEntry):
    ...
    unsubs = [
        async_track_state_change_event(hass, [eid], _on_change)
        for eid in WATCHED
    ]
...
async def _send(hass: HomeAssistant, entry: ConfigEntry, title: str, message: str):
    ...
    if service:
        ...
        try:
            await hass.services.async_call(domain, svc, {"title": title, "message": message})
            return
        except Exception as err:
            _LOGGER.warning("PumpSteer notify failed (%s), using persistent_notification: %s", service, err)
