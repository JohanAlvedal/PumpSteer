...
# class PumpSteerMLSensor(Entity):
#     """Sensor displaying insights from the PumpSteer ML system."""
...
#         elif self._attributes.get("collecting_data", False):
#             return "mdi:robot-industrial"
#         elif self._state in ["-", "unknown"]:
...
#             success = perf.get("success_rate")
#             duration = perf.get("avg_heating_duration")
#             efficiency_score = None
#             if isinstance(success, (int, float)) and isinstance(duration, (int, float)) and duration > 0:
#                 efficiency_score = round(success * 100 / duration, 1)

#             if isinstance(success, (int, float)):
#                 self._state = f"{success:.0f}%"
#             elif efficiency_score is not None:
#                 self._state = f"Eff:{efficiency_score}"
#             else:
#                 self._state = "-"

#             # Calculate time since last gain adjustment
#             try:
#                 if last_gain_adjust and last_gain_adjust not in ["-", "unknown"]:
#                     dt = dt_util.parse_datetime(last_gain_adjust)
...
#             self._attributes = {
#                 "success_rate": round(success, 2) if isinstance(success, (int, float)) else None,
#                 "avg_heating_duration": duration if isinstance(duration, (int, float)) else None,
#                 "most_used_aggressiveness": perf.get("most_used_aggressiveness") or None,
#                 "total_heating_sessions": perf.get("total_heating_sessions") or None,
#                 "recommendations": recs or "Still collecting data...",
#                 "auto_tune_active": autotune_on,
...
#                 "integral_gain": round(gain, 3) if isinstance(gain, (int, float)) else 0,
#                 "last_gain_adjustment": last_gain_adjust if last_gain_adjust not in [None, "unknown"] else "-",
#                 "hours_since_last_adjustment": hours_since,
#                 "ml_efficiency_score": efficiency_score,
#                 "ml_status": status.get("ml_status") or "-",
#                 "collecting_data": status.get("collecting_data", False),
#                 "sessions_collected": status.get("sessions_collected", 0),
#                 "last_updated": dt_util.now().isoformat()
...
#     async_add_entities: AddEntitiesCallback,
# ) -> None:
#     sensor = PumpSteerMLSensor(hass, config_entry)
