...
class PumpSteerSensor(Entity):
    ...
    def _calculate_output_temperature(
        ...
        return (
            ...
            mode,
            {
                ...
                "forecast_feedforward_raw_c": forecast_feedforward_raw_c,
    ...
    def _calculate_pi_output(
        ...
        price_feedforward_c = price_bias * price_feedforward_gain
        forecast_feedforward_raw_c = forecast_bias * forecast_feedforward_gain
        forecast_feedforward_c = self._apply_forecast_heating_gating(
            forecast_component=forecast_feedforward_raw_c,
            indoor_temp=indoor_temp,
            control_target_temperature=target_temp,
            outdoor_temp=outdoor_temp,
        )
        feedforward_bias = price_feedforward_c + forecast_feedforward_c
    ...
    def _apply_forecast_heating_gating(
        self,
        forecast_component: float,
        indoor_temp: float,
        control_target_temperature: float,
        outdoor_temp: float,
    ) -> float:
        """Gate heating-oriented forecast feedforward by comfort and season."""
        if forecast_component >= 0.0:
            return forecast_component

        # Comfort gating: do not preheat when comfort target is already met.
        if indoor_temp >= control_target_temperature:
            return 0.0

        # Mild-weather gating: keep winter preboost, fade out in milder weather.
        if outdoor_temp <= -5.0:
            weather_scale = 1.0
        elif outdoor_temp >= 10.0:
            weather_scale = 0.0
        else:
            weather_scale = (10.0 - outdoor_temp) / 15.0

        return forecast_component * weather_scale
    ...
    def _build_attributes(
        ...
        attributes = {
            ...
            "forecast_feedforward_raw_c": (
                round(control_debug.get("forecast_feedforward_raw_c"), 3)
                if control_debug and control_debug.get("forecast_feedforward_raw_c") is not None
                else None
            ),
            "feedforward_total_c": (
