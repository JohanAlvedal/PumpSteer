...
def validate_price_list(
    ...
    if extreme_prices:
        _LOGGER.warning(
            f"Found {len(extreme_prices)} extremely high prices (>{MAX_PRICE_WARNING_THRESHOLD} kr/kWh)"
        )
...
def classify_prices(
    ...
    if not validate_price_list(price_list):
        _LOGGER.debug(
            f"Invalid price list for classification (length: {len(price_list) if price_list else 0})"
        )
