...
_ha = _make_module("homeassistant", package=True)
_ha_util = _make_module("homeassistant.util", package=True)
_ha_dt = _make_module("homeassistant.util.dt")
_ha_core = _make_module("homeassistant.core")
_ha_components = _make_module("homeassistant.components", package=True)
...
_ha_core.callback = lambda func: func  # type: ignore[attr-defined]
_ha_weather.ATTR_FORECAST = "forecast"  # type: ignore[attr-defined]
...
# Clean up all temporary homeassistant stubs so other tests see the real modules.
sys.modules.pop("pumpsteer_forecast", None)

for name in list(sys.modules):
    if name == "homeassistant" or name.startswith("homeassistant."):
        if name not in _ORIGINAL_HA_MODULES:
...
def _point(
    hour: int, temp: Optional[float], wind: float = 0.0, day: int = 1
) -> ForecastPoint:
...
def _cold_night_warm_day() -> list:
    ...
    return [
        _point(h, 2.0 if (h >= 22 or h < 6) else THRESHOLD + 3.0) for h in range(24)
    ]
...
class TestAnalyzeEmpty:
    def test_empty_points_returns_safe_defaults(self):
        o = analyze_thermal_outlook([], THRESHOLD)
...
class TestNightDaySplit:
    def test_night_min_reflects_night_hours(self):
        o = analyze_thermal_outlook(_cold_night_warm_day(), THRESHOLD)
    ...
    def test_all_cold_night_and_day(self):
        o = analyze_thermal_outlook(_cold_night_cold_day(), THRESHOLD)
    ...
    def test_none_temps_excluded(self):
        ...
        o = analyze_thermal_outlook(points, THRESHOLD)
...
class TestHoursBelowThreshold:
    def test_all_cold(self):
        o = analyze_thermal_outlook([_point(h, 5.0) for h in range(24)], THRESHOLD)
        assert o.hours_below_threshold == 24

    def test_all_warm(self):
        o = analyze_thermal_outlook(
            [_point(h, THRESHOLD + 1.0) for h in range(24)], THRESHOLD
        )
        assert o.hours_below_threshold == 0

    def test_mixed(self):
        points = [_point(h, 5.0) for h in range(12)] + [
            _point(h + 12, THRESHOLD + 1.0) for h in range(12)
        ]
        o = analyze_thermal_outlook(points, THRESHOLD)
        assert o.hours_below_threshold == 12

    def test_none_temps_not_counted(self):
        points = [_point(h, None) for h in range(6)] + [
            _point(h + 6, 5.0) for h in range(6)
        ]
        o = analyze_thermal_outlook(points, THRESHOLD)
...
class TestTrendDetection:
    def test_rising_sets_warming_trend(self):
        o = analyze_thermal_outlook(_rising_temps(), THRESHOLD)
        assert o.warming_trend is True
        assert o.cooling_trend is False

    def test_falling_sets_cooling_trend(self):
        o = analyze_thermal_outlook(_falling_temps(), THRESHOLD)
        assert o.cooling_trend is True
        assert o.warming_trend is False

    def test_stable_no_trend(self):
        o = analyze_thermal_outlook([_point(h, 5.0) for h in range(6)], THRESHOLD)
        assert o.warming_trend is False
        assert o.cooling_trend is False
    ...
    def test_too_few_points_no_trend(self):
        o = analyze_thermal_outlook([_point(h, 5.0) for h in range(3)], THRESHOLD)
        assert o.warming_trend is False
        assert o.cooling_trend is False
...
class TestPrecoolRisk:
    def test_warm_day_triggers_risk(self):
        o = analyze_thermal_outlook(_warm_summer_day(), THRESHOLD)
    ...
    def test_just_below_margin_no_risk(self):
        ...
        o = analyze_thermal_outlook(points, THRESHOLD, precool_margin=3.0)
        assert o.precool_risk is False
...
class TestPreheatWorthwhile:
    ...
    def test_fewer_than_3_cold_hours_suppresses_preheat(self):
        points = [_point(h, 5.0) for h in range(2)] + [
            _point(h + 2, THRESHOLD + 1.0) for h in range(22)
        ]
        o = analyze_thermal_outlook(points, THRESHOLD)
        assert o.preheat_worthwhile is False
...
class TestPreheatStrength:
    ...
    def test_scales_with_coldness(self):
        cold = analyze_thermal_outlook([_point(h, 2.0) for h in range(24)], THRESHOLD)
        mild = analyze_thermal_outlook([_point(h, 12.0) for h in range(24)], THRESHOLD)
        assert cold.preheat_strength > mild.preheat_strength

    def test_clamped_to_1(self):
        o = analyze_thermal_outlook([_point(h, -20.0) for h in range(24)], THRESHOLD)
        assert o.preheat_strength <= 1.0

    def test_never_negative(self):
        o = analyze_thermal_outlook(
            [_point(h, THRESHOLD - 0.1) for h in range(24)], THRESHOLD
        )
        assert o.preheat_strength >= 0.0
...
class TestEffectiveTempNow:
    def test_no_wind_equals_raw(self):
        o = analyze_thermal_outlook(
            [_point(h, 5.0, wind=0.0) for h in range(6)], THRESHOLD
        )
        assert o.effective_temp_now == pytest.approx(5.0)

    def test_wind_reduces_effective_temp(self):
        o = analyze_thermal_outlook(
            [_point(h, 5.0, wind=10.0) for h in range(6)], THRESHOLD
        )
...
class TestScenarios:
    ...
    def test_summer_extreme_heat(self):
        ...
        o = analyze_thermal_outlook([_point(h, 32.0) for h in range(24)], THRESHOLD)
        assert o.precool_risk is True
        assert o.preheat_worthwhile is False
        assert o.preheat_strength == 0.0
    ...
    def test_windy_cold_evening(self):
        ...
        o = analyze_thermal_outlook(
            [_point(h, 3.0, wind=12.0) for h in range(6)], THRESHOLD
        )
        assert o.effective_temp_now is not None
