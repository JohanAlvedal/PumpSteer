...
- Triggers when expensive period is coming **AND** forecast is cold
...
In 2.1.0, this sensor is **diagnostic only** — it does not yet replace the `_forecast_is_cold()`
heuristic used in block 5b. Connecting `ThermalOutlook.preheat_worthwhile` to block 5b
